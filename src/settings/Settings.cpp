...
bool SettingsStore::save(const InputMap& map, content::LoadReport& report) const {
    ...
    if (ec) {
        report.add(kSource, "<file>",
                   "could not create settings directory: " + file_.parent_path().string());
        return false;
    }
    std::ofstream out(file_, std::ios::binary | std::ios::trunc);
    if (!out) {
        report.add(kSource, "<file>", "could not open settings file for writing");
        return false;
    }
    out << serializeSettings(values, map);
    if (!out) {
        report.add(kSource, "<file>", "failed while writing settings file");
        return false;
    }
    return true;
}
