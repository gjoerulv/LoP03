...
TEST_CASE("loader: shipped data loads with zero errors", "[content][data]") {
    ...
    REQUIRE(db.loreQuestionCount() == 28);  // M112: the Jester's pool, four a tier
    REQUIRE(db.cutsceneCount() == 22);  // M97's 8-scene Hooded Goose arc (file is
                                        // REQUIRED) + 7 M100 jokes + 3 M103 stories
                                        // + 4 M110 patrol scenes
