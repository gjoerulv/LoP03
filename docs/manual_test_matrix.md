...
| 241 | The painted battle stages (M119) | fights in every theme (a plain patrol and a boss), a castle challenge, then the same with High Contrast on | not run | not run | each band shows a far room under the ink silhouettes (Keep: wall, slits, tower; Mine: crystal roof, clusters, rails; Forest: trunks, canopy, roots; Castle: windows, banners, chandelier, dais; Goosy: reeds, mist, pond) and the centre corridor stays plain so floats, status lines and meters read exactly as before; under High Contrast the painting is gone and the band looks as it did in M56. **Judge that the fight is never harder to read** | floats/status text over the corridor | battle/boss music | 18_battle_boss.png, 31_battle_high_town.png, 120_battle_goosy.png, 130_battle_golden_goose.png, 140_dragon_clone.png | not run |
