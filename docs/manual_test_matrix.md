...
| 228 | Formation with tall units (M115) | any boss fight with a full court (the Rush Tyrant, a Guild Master's court, an Endless boss wave), and a plain 5-enemy patrol | not run | not run | rows sit exactly where they did before (the boss at centre with the two rows above lifted by the old headroom); nothing climbs off the top of the screen; a 5-enemy fight still starts higher; target cycling walks top-to-bottom | no overlaps anywhere | n/a | 24_battle_boss.png, 31_battle_high_town.png | not run |
