...
| 96 | Losing a castle challenge (M43) | lose the Boss Rush partway, then lose to the King | not run | not run | the defeat message claims no gold loss; the result panel counts the real roster ("X of 12 bosses"), says you are patched up at the gates, and the party is at full HP/MP back at the castle; judge whether the free heal feels right | n/a | defeat jingle | 35_castle_result.png | not run |
