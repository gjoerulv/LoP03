...
| 194 | Center-out formation (M101) | fight a 1-enemy, a 3-enemy and a 5-enemy team, then a boss with court; kill mid-column enemies and keep fighting; use single-target skills and items | not run | not run | the first enemy (the BOSS when one exists) stands at the CENTER row, then center-top, center-bottom, top, bottom in that order; a lone enemy stands centered; fallen enemies keep their row (no reflow); target cycling walks top-to-bottom visually; damage numbers, Miss/Weak/Immune marks and element accents sit on the right sprites. **Judge the composition — the boss flanked by its court** | rows never overlap; 5-row fights start higher as before | battle sfx | n/a | not run |
...
| 197 | Pond spirit & the forge (M103) | sweep seeds until the Pond Spirit and the Hungry Forge appear; accept both; fight on; end runs each way (boss win, retreat, wipe) with a goosed member | not run | not run | the pond names a RANDOM non-goose member who fights as a bare Goose (heirloom kept, arms gone) for the REST of the run; +100 itemized as "Goose pact"; XP/levels earned while goosed survive the restore; a KO'd goose comes back KO'd; an all-geese party gets a dry refusal and keeps the event; the forge lists only BAG equipment, the piece is gone, and the NEXT victory panel shows doubled XP exactly once. **Judge the pact's fun and the 6-8% rates** | choice modal + outcomes fit | interact/status sfx | n/a | not run |
...
| 202 | The Goosy Gauntlet (M106) | at towns 1-6: confirm three themes only; at town 7: pick Goosy Gauntlet and run it; meet the Flock rite and fight its battle; fight all three new bosses across seeds; run the Boss Rush; dig a low-town treasure | not run | not run | the theme lists only at town 7; runs read pond-y (interim forest tiles/music — judge acceptability); pond-fowl enemies occasionally just "Honk."; the Flock turns the WHOLE party to geese for one fight (heirlooms kept, arms shed) and the reckoning pays +300 as "Flock battles"; the Gander enrages, the Mother heals-and-hexes, the Pondlord commands, all with Evil Goose courts; Boss Rush runs 15 fights; a town-1 dig's guard is never a Goosy boss. **Judge the interim art call: keep, or commission a Goosy art+audio milestone** | rite panel + telegraphs fit | forest pair (interim) | n/a | not run |
...
| 204 | Are P Geese (M108) | build fresh; check the window title, title screen and Explorer properties of ArePGeese.exe; rename your %APPDATA%\ArePGeese away, keep CrystalDungeons, launch (migration), relaunch (no-op); run tools\package.ps1 | not run | not run | everything player-facing says "Are P Geese" (window, title screen, fatal dialog, welcome prompt, VERSIONINFO); first launch with only a legacy folder copies every save/config file into ArePGeese and logs it, the OLD folder untouched; a second launch copies nothing; a populated new folder always wins; the package stages ArePGeese-<version>-win64.zip; the ICON is still the crystal and the title one-liners unchanged (deliberate interim). **Judge the name on screen, and whether the icon/one-liner art pass should be commissioned now** | title fits everywhere it renders | n/a | 01_title.png | not run |
