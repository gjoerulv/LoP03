...
| 117 | Battle-rules tag (M47) | clear a dungeon, then open the Scoreboard | not run | not run | the run is tagged with battle rules v7; older entries keep their own version and are flagged as played under different rules (never renormalized) | n/a | n/a | 12_scoreboard.png | not run |
