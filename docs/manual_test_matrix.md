...
> localized long-prose stress; **127** as of the 2026-08-29 polish round;
> **148** as of M116).
> The M87 note §G is the checklist. One deliberate M87 deviation since
> 2026-08-17: the battle command panel's skill/item preview is three lines
> with NO more-arrow (no scroll exists there and [Details] is already in
> the header) — see the M87 note's policy adjustment.
>
...
> (`build-msvc\ArePGeese.exe --capture docs\screenshots\m23_captures`)
...
| 2 | Title: Continue disabled | no save files (rename `%APPDATA%\ArePGeese\saves`) | not run | not run | not run | not run | — | 07_title_no_saves.png | not run |
...
| 109 | Goose in battle (M45) | take a Goose into a fight and use its heals; reach level 30 for the ultimate | not run | not run | the heal works AND buffs every enemy (visible tags on the foes); nothing can be equipped; the level-30 ultimate (Everyone Is Welcome) poisons every foe for 30 MP — the shipped skill is the design (owner ruling 2026-09-10) | goose sprite | heal sfx | - | not run |
...
| 181 | The sparring mirror (M94; crash fix 2026-08-17) | Training Hall → arrow DOWN from the last member onto "Spar: face your echoes" and pause there (the 2026-08-17 crash repro — must NOT assert/crash; the portrait simply disappears on the spar rows); then run both modes: win one, lose one, flee one; "manual control" and command both sides; check HP/MP/items/gold before vs after each | not run | not run | hovering either spar row is safe and drops the portrait (no member is highlighted); the echoes mirror the party exactly (names "Echo X", same gear share/skills); the AI drives them in mode 1; in manual mode echo turns use the normal command phases with Item and Escape disabled; after ANY outcome the party is byte-identical to before the spar (HP, MP, bag, gold, records, bestiary); the closing modal reads "no harm done"; no XP, no spoils panel leakage. **Judge both modes' feel** | closing modal fits | battle sfx; confirm on close | 111_spar_closing.png, 122_training_spar_row.png | not run |
...
| 183 | Heirlooms (M96; UI fixes 2026-08-17) | debug "Grant 1x each heirloom"; equip several via Equip Party (incl. on a Goose if unlocked); fight with the Emberwake Locket (drop below 50%), the Lastlight Band (fight at ≤25% HP, then heal back over), and an ally-felled piece; save/reload; revisit the Stranger's offer and the Party screen | not run | not run | the equip flow shows the fourth Heirloom slot for every class; the Stranger's offer box shows each keepsake's FULL description (the box grows to the longest option); the Party screen lists "Heirloom: <name>" under the gear lines and Details carries its effect text; the Locket mends 15% exactly once at first drop below half (announced in the message + log); the Band's edge bites only while at/below the line and sheathes when healed past it (announced once); ally-felled and every-Nth pieces fire as described; heirlooms never appear in shop Buy lists; the equipped piece survives save/reload. **Judge effect strength vs accessories** | announcements read cleanly | battle sfx | 113_cutscene_choice.png, 79_party_panel.png | not run |
...
| 185 | Prologue before prompts (M98) | start a New Game; watch the prologue end-to-end; then arrive at town 2 for the first time (its scene plays) | not run | not run | NO tutorial prompt appears during or over any cutscene; the town-welcome prompt appears immediately AFTER the prologue closes (town 1, standing in the plaza); the first-travel prompt appears after town 2's arrival scene closes; each still fires exactly once per save. **Judge that the story now breathes first** | prompt panel unchanged | confirm sfx | n/a | not run |
| 186 | Equip shop count + heirloom text (M98) | buy 2-3 pieces back to back in one shop visit watching the "x N" column; then Equip Party → a member's Heirloom slot → highlight an heirloom candidate | not run | not run | the owned count updates the moment a purchase lands (no shop re-entry needed), cursor and "Bought ..." banner stay put; an heirloom candidate shows its NAME + EFFECT text in the band where the stat diff sits (no more "HP 0 ATK 0 ..." row); other slots' diff rows are unchanged; the worn heirloom is still readable one step back on the slot row. **Judge the band's readability** | two-line wrap never clips (capture 115) | confirm sfx | 115_equip_heirloom_text.png | not run |
| 187 | Arcane Burst + elemental voices (M98) | teach/select Arcane Burst and cast it; cast Fireball, Frost Lance, Spark, Stone Edge in one battle; check the session log | not run | not run | Arcane Burst hits as DARK (weak/immune readouts + the dark wisp accent + the dark breath voice); each element sounds its own voice distinct from the plain magic chime; the log carries "elemental impact SFX loaded from files: 6/6" and NO "remapping to hit_magic" warnings. **Judge whether the six voices are audibly distinct — if not, the M98 audit says the WAV designs need an owner-directed redesign, not the code** | n/a | THE test | n/a | not run |
| 188 | The finale's true King (M98) | with kingDefeated set (or debug "Play cutscene" → finale), watch the finale's staged King | not run | not run | the dimmed King on stage is THE Hollow King's battle sprite (the castle boss), not the Hollow Sovereign dungeon boss; the Dragon is unchanged; the fallback silhouette only appears if the texture is missing. **Judge the resemblance** | staging unchanged | n/a | 114_cutscene_finale.png | not run |
| 189 | Tutorial truth pass (M99) | Settings → reset tutorial prompts (or a fresh save); replay the dungeon-entry, town-return, first-travel, first-castle, result and guild prompts | not run | not run | every prompt tells the truth: the dungeon beat teaches the Patrol counter (no "nothing ambushes you"); the town beat prices the Inn in gold with the rest-token alternative; travel is the east/west edge walk-through; the castle beat counts four challenges without naming the Dragon; the reckoning compares town, depth AND level; the Guild beat mentions Floors. **Judge usefulness, not just truth** | all six wrap inside the panel | confirm sfx | n/a | not run |
| 190 | Forge-edited tutorials (M99) | CrystalForge → Tutorials → edit a beat's body; save; validate; relaunch the game; reset tutorials; trigger that beat | not run | not run | the Tutorials category lists all 16 beats by title; edits validate through the real loader and save in canonical single-line form (clean git diff); the game shows the edited text; deleting the entry (or the whole file) falls back to the built-in text rather than silencing the prompt. **Judge the editing flow** | n/a | n/a | n/a | not run |
| 191 | THE STRANGER "P" (M100) | play the prologue and any town scene; open the finale choice; stand by the roadside NPC | not run | not run | every dialogue caption reads `- THE STRANGER "P" -`; the choice plaque reads `The Stranger "P" Offers`; the roadside nameplate is `"P"`; party name tokens still resolve. **Judge whether the joke lands** | caption fits the plaque line | n/a | 112_cutscene_dialogue.png | not run |
| 192 | The taller stage (M100) | read the prologue and the finale end to end at normal message speed | not run | not run | the actors stand higher, the panel below grows up to six lines, and NO shipped beat needs scrolling (the scroll hint only appears if a beat ever exceeds six lines); skip-ask and the choice modal sit over the dimmed stage as before. **Judge readability and stage composition** | no beat scrolls; nothing clips | confirm sfx | 114_cutscene_finale.png | not run |
| 193 | Finale once, then jokes (M100) | finish the finale (record its keepsake); talk to the NPC 8+ more times; save/reload mid-cycle; separately: quit mid-finale before choosing, then return | not run | not run | after the choice is recorded the finale NEVER replays; each visit tells ONE dry joke (2-3 beats, no choice, no reward), first the owner's wild-goose-chase line, cycling through all seven before any repeats; the cycle position survives save/reload; a quit-mid-finale first play re-offers the finale until the keepsake is chosen. **Judge the jokes' dryness** | joke beats fit unscrolled | confirm sfx | 116_stranger_joke.png | not run |
| 194 | Center-out formation (M101; boss headroom fix 2026-08-17) | fight a 1-enemy, a 3-enemy and a 5-enemy team, then a boss with court; kill mid-column enemies and keep fighting; use single-target skills and items | not run | not run | the first enemy (the BOSS when one exists) stands at the CENTER row, then center-top, center-bottom, top, bottom in that order; a lone enemy stands centered; fallen enemies keep their row (no reflow); target cycling walks top-to-bottom visually; damage numbers, Miss/Weak/Immune marks and element accents sit on the right sprites; in EVERY boss-with-court fight the minion above the boss (sprite AND HP meter) sits fully clear of the boss's 36px crown — no overlap (fix 2026-08-17: the two rows above the boss lift 10px). **Judge the composition — the boss flanked by its court** | rows never overlap; 5-row fights start higher as before | battle sfx | 121_battle_goosy_boss.png | not run |
| 195 | The fallen earn nothing (M102) | let one member fall, win the fight (ideally with enough XP that they would have leveled); check the victory panel, the party screen, and their HP; then revive via Phoenix Tear from the field menu and via the Inn | not run | not run | the fallen member gains NO XP and no level; they are NOT standing after the fight (no more silent level-up revive); the victory panel lists diffs only for the living; Phoenix Tear (field or battle), Renew and the Inn still raise them; the Elder Root and Training Hall still teach everyone (paid tuition is exempt — judge whether that feels right). **Judge the harshness** | panel reads honestly | battle sfx | n/a | not run |
| 196 | The shelf lost its scrolls (M102, gen v18) | open chests and visit the Duck Peddler across several runs at depth (town 3+ where scrolls used to appear); complete a raised-stakes 20F for the trove; dig a town treasure | not run | not run | no chest and no peddler offer is ever a skill scroll; the Guild trove and the town digs still pay them; the scoreboard tags new runs generation v18 (older entries stay comparable among themselves). **Spot-check, not exhaustive** | n/a | n/a | n/a | not run |
| 197 | Pond spirit & the forge (M103; modal window fix 2026-08-17) | sweep seeds until the Pond Spirit and the Hungry Forge appear; accept both; fight on; end runs each way (boss win, retreat, wipe) with a goosed member | not run | not run | the pond names a RANDOM non-goose member who fights as a bare Goose (heirloom kept, arms gone) for the REST of the run; +100 itemized as "Goose pact"; XP/levels earned while goosed survive the restore; a KO'd goose comes back KO'd; an all-geese party gets a dry refusal and keeps the event; the forge lists only BAG equipment, the piece is gone, and the NEXT victory panel shows doubled XP exactly once. the forge's pick list shows at most nine rows in a scrolling window (arrows mark the rest) under a two-line prompt - a deep bag can no longer push the box off screen. **Judge the pact's fun (rates are the shared 8% since gen v23 - row 205)** | choice modal + outcomes fit | interact/status sfx | n/a | not run |
| 198 | Altar, hood & changer (M103) | meet the Level Altar (once with a level-99 member), A Familiar Hood, and the Token Changer (with and without a legendary token) | not run | not run | the altar grants exactly one level and empties that member's MP; at 99 it spends itself on a dry line only; the hood tells one short story (one of three, seeded per room) then grants 20 MP to your pick; the changer trades 1 legendary token for 3 rest tokens or 1 map piece (M83 banking honored) and refuses the tokenless without spending. **Judge the stories' register** | story beats fit the six-line panel | heal/interact sfx | n/a | not run |
| 199 | The cold trail (M103) | walk the patrol counter down to ~30, then use the Cold Trail | not run | not run | the "Patrol N" chip stands at 100 again and keeps ticking normally afterwards; the event resolves once. **Trivial but satisfying — the rate is the shared 8% since gen v23 (row 205)** | chip unchanged | interact sfx | n/a | not run |
| 200 | The gambling den (M104; reels icons 2026-08-17) | find the Reels and the Dealer (the shared 8% each since gen v23; sweep seeds); spin once and thrice; land a match (or debug-check several seeds); play blackjack to a win, a loss, a push and a dealt 21; try both dens broke; try the spoon/crown prize at cap | not run | not run | the reels are ONE play (1 spin 10g or 3 for 70g, then gone, win or lose); each spin renders as a ROW OF THREE ICONS at 2x (papers, goose head, spoon, crown, red X, bald-red-beard, gold 7) with the prize text below; matches pay the owner's table exactly (tax clamps at your gold; goose head jokes then pays this-town-new gear; spoon/crown respect caps with a dry refusal; red X banks a map piece; bald head pays ONE normal scroll; 7s pay 1000g); blackjack deals seeded hands, Confirm hits / Cancel stands, dealer stands 17, a win returns the bet doubled, a push returns it, one round per den; broke parties are refused without spending the event; a reload replays identical reels/cards. **Judge the odds' feel, the icon readability, and whether a spin animation is wanted** | panel grows one icon row per spin and still fits with 3 | chest/interact sfx | 119_reels_icons.png | not run |
| 201 | The Eternal descent (M105) | at town 7: cycle Floors to Eternal (towns 1-6 must not offer it); try adjusting Depth; read the warning and decline once; enter and retreat instantly, then check the Guild's stakes forewarn on a normal run; descend several floors; save/reload after; wipe once | not run | not run | the stepper reads "Eternal" only at town 7 and Depth locks at "20 (fixed)"; the warning states no-score + boss-every-floor + the unconditional stakes raise; even an instant retreat leaves the baseline at (town 7, depth 20) — the forewarn on a lower run now shows the penalty; every floor's stairway is guarded by a REAL boss whose danger tier climbs (+10 %pts/floor); the "Eternal N" chip tracks the floor; each felled floor-boss updates "Eternal best" live and it survives save/reload; defeat is the normal carry-out with the record kept. **Judge the escalation pace, the stakes price, and whether a dedicated result screen is missed** | warning fits the prompt panel | door/battle sfx | n/a | not run |
| 202 | The Goosy Gauntlet (M106; own art/audio 2026-08-17) | at towns 1-6: confirm three themes only; at town 7: pick Goosy Gauntlet and run it; meet the Flock rite (an 8% encounter-tier roll - sweep seeds) and fight its battle; fight all three new bosses across seeds; run the Boss Rush; dig a low-town treasure | not run | not run | the theme lists only at town 7; the dungeon walks on still-water floors between reed-palisade walls with parted-reed doors and a nest shrine; battles stage on the reed-and-pond backdrop with the ELEVEN new sprites (shovel-billed drake, periscope honker, oar marauder, shawled Hag, splash imp, bomb grenadier, swan cob, horn herald; the Gander at full height, the veiled Mother on her nest, the Pondlord with reed diadem + cattail standard) — no evil-goose repeats; the waddling strut plays and the wetland bed honks in the distance; pond-fowl enemies occasionally just "Honk."; the Flock turns the WHOLE party to geese for one fight and the reckoning pays +300 as "Flock battles"; boss courts are the Evil Geese (theirs by design); Boss Rush runs 15 fights; a town-1 dig's guard is never a Goosy boss. **Judge the new art, tune and bed — feel is yours** | rite panel + telegraphs fit | the strut + wetland bed | 118_dungeon_goosy.png, 120_battle_goosy.png, 121_battle_goosy_boss.png | not run |
| 203 | Summon stagecraft (M107) | teach the three legends (debug grant); cast each in battle at Normal and Fast message speed, with Battle Flash Full and Off | not run | not run | the summoned creature appears LARGE at the battlefield's center for the resolution beat — the Goose as its true sprite writ big with a golden burst and bob, the Sentinel as falling star-streaks into a bright lattice, the Spring as rising ripples — each with the new arrival fanfare; Flash Off strips the burst but keeps the creature; Fast shortens the beat; combatants, floats and panels stay readable beneath; the quip line still names the legend. **Judge "epic relative to the game's feel" — the milestone's whole point — and whether per-summon fanfares are wanted** | apparition never covers the command panel | the new summon fanfare | 117_summon_goose.png | not run |
| 204 | Are P Geese (M108; title/icon fix 2026-08-17) | build fresh (CLEAN configure so the rc re-runs); check the window title, the TITLE SCREEN, and Explorer properties of ArePGeese.exe; rename your %APPDATA%\ArePGeese away, keep CrystalDungeons, launch (migration), relaunch (no-op); run tools\package.ps1 | not run | not run | everything player-facing says "Are P Geese" — window, the title plaque now reading ARE P GEESE under the new GOOSE EMBLEM (a goose on still water), fatal dialog, welcome prompt, VERSIONINFO; the exe carries the goose .ico (Explorer's icon cache may lag — file properties are authoritative) and stamps the current project version bottom-left (v0.7.0 at M108; v0.8.0 since the 2026-09-10 distribution; v0.9.0 since the 2026-09-18 one); first launch with only a legacy folder copies every save/config file into ArePGeese and logs it, the OLD folder untouched; a second launch copies nothing; a populated new folder always wins; the package stages ArePGeese-<version>-win64.zip (0.9.0 today) with the rebranded README/LICENSES headers; the title one-liners are unchanged (owner scope). **Judge the emblem and icon** | title fits everywhere it renders | n/a | 01_title.png | not run |
| 205 | Equal event weighting (gen v23, 2026-08-28) | play a few long runs (20F ideal) across themes and note which encounters turn up; run a multi-floor descent for the Surveyor; sweep seeds for any two specific encounters | not run | not run | across a few 20-floor runs every encounter kind — the peddler, dragonform, the pond spirit, the forge, the altar, the hood, the changer, the cold trail, both dens, and the theme's own rite — shows up at a similar, unremarkable rate (~1-2 sightings each per 20F run; the Duckling Peddler no longer dominates); the Surveyor appears reliably on fogged descents (~1 floor in 4) and never on a 1F map; the Royal Relic's cadence feels unchanged; a reload reproduces every event exactly. **Judge the new pacing: encounters equally rare, staples still carrying the run** | n/a | n/a | n/a | not run |
| 206 | Gear names & the slot info band (2026-08-28) | open a chest holding gear; buy the peddler's duckling; win a reels prize; burn a piece at the Hungry Forge; meet the Stranger's keepsake offer (new game); open Equip Party and walk all four slot rows on an outfitted member and an empty one | not run | not run | every found/traded piece appears on its own centered line in the outcome panel — its M81 icon (gear/relics/heirlooms) + the name in gold — and the sentence no longer restates the name; the Stranger's offer shows the keepsake as icon + gold name with the description below; the Hungry Forge's offering list leads each row with its icon; in Equip Party the slot list now carries a bottom info band describing the hovered slot's piece (stats/resistance/description; an heirloom's effect text) or "Nothing equipped in this slot."; nothing overlaps the footer. **Judge the gold-name treatment: standing out without shouting** | outcome tags + the two-line slot band fit | chest/interact sfx | 125_equip_slot_info.png, 119_reels_icons.png, 113_cutscene_choice.png | not run |
| 207 | The Jester echo commands itself (2026-08-29) | bring a Jester in the party; spar in MANUAL mode until the Jester echo's turn comes up several times; then spar once in AI mode | not run | not run | in manual sparring every echo takes your commands EXCEPT the Jester echo, which acts entirely on its own (attacks, skills, the occasional quip) exactly like the Jester in your own ranks; forced-status echo turns still auto-resolve; the both-sides-AI spar plays out exactly as it did before the fix | the command panel never opens for the Jester echo | quips as usual | n/a | not run |
| 208 | New Game is a clean slate (2026-08-29) | load a save with real progress (Guild Boss beaten + perk chosen, castle road open, curios, map pieces, a full bestiary); quit to title from the party menu; start a New Game; visit the Guild, the bestiary, the Maps screen; try walking to town 2 | not run | not run | the fresh game carries NOTHING over: "Fight the Guild Boss" locked in every town and no perk bonuses active (watch XP/gold spoils), the castle road shut, Goose Town shut, zero map pieces/curios/dug scrolls, an empty bestiary, zeroed victory records, the story serial unheard, stakes fresh, town 2 locked; reward-class unlocks and achievements persist (cross-game by design, stored outside the save) | n/a | n/a | n/a | not run |
| 209 | The footer prompt ellipsizes (2026-08-29) | face an elite-challenge event whose full prompt is long (e.g. "Challenge Feral Marauders - Easy x2 - double danger score, no treasure" at low depth); read the footer, then Confirm to the event panel | not run | not run | a prompt too long for the strip ends in "..." (no text runs off screen and no [ui-overflow] warning appears in the console); the event panel one Confirm away shows the complete trade-off; short prompts render exactly as before | the "..." is the policy-E marker (ui_style_guide S7) | n/a | n/a | not run |
| 210 | Long target names beat the button hints (2026-08-29) | in battle, target foes with long names (the town-2 Guild Master among them) and short ones | not run | not run | when a target's name would collide with the right-side Back hint, the hint hides and the name takes the row; short names keep the hint exactly as before; Cancel still backs out either way | name and hint never overlap | n/a | n/a | not run |
| 211 | The dens got a show (2026-08-29) | play the Reels (1 and 3 spins; try skipping mid-spin) and Blackjack (hit to a bust, stand to a dealer reveal) | not run | not run | the reels SPIN: cells flick and land left to right, row by row, a tick per landed cell, prizes/tags/Continue held back until the last cell (Confirm skips to the landed result; message speed paces it; a reload replays the identical spin); blackjack deals real cards - the hole card face-down on the woven back until the hand ends, ranks lettered on cream faces, the goose Jack / duck Queen / dark-king King / crystal Ace for the royalty; values sit beside the labels. **Judge the spin pace and the card faces** | spin frame + card table fit the panel | lock ticks while landing | 126_reels_spin.png, 127_blackjack_cards.png, 119_reels_icons.png | not run |
| 212 | Party creation shows the classes (2026-08-29) | on New Game: cycle classes on every row (locked ones too); press the Details key on several classes; start a run | not run | not run | every roster row leads with its class's battle sprite, changing the instant the class cycles (locked classes grey to a silhouette); the Details key opens the class's full sheet - role, base stats, per-level growth, starting skills, learnset with levels, equip bans, quirks (the Jester's self-command note, the Dragon's sweep), score modifier, and the lock note; the footer advertises "Info" on the Details key. **Judge whether the sheet says enough to pick with confidence** | sheet scrolls if long (policy C) | confirm sfx on open | n/a | not run |
| 213 | Lifetime ledger (M109) | dev build: play a run (a fight, a chest, an event, a patrol, a retreat, a wipe), shop / rest / train in town, save and reload; then load a pre-M109 save; then spar once | not run | not run | nothing in play looks or feels different (the ledger is silent); the debug menu's read-only **Lifetime** row shows the fight count and play time growing across the session and surviving a reload; the pre-M109 save shows zero fights with its old biggest hit carried over; the play time stops while a pause menu, Settings or the main menu is open; a spar leaves every number as it was | n/a | n/a | 65_debug_menu.png | not run |
| 214 | The patrol mixture (M110) | dev build, a long run: walk patrols down naturally (and with **Debug → Trigger patrol now**); note what answers each time; reload the entry autosave and walk the same patrols again | not run | not run | most patrols are the familiar Roused Patrol; now and then THE STRANGER "P" steps out instead (one of four short new tales — no question, no reward, no fight; Cancel skips as any scene) and the Patrol chip is back at 100 without a battle; a reload replays the SAME kinds in the same order; **Trigger patrol now** fires the instant the menus close (no tile walk); the new scoreboard entries tag generation **v24**. **Judge the tales' dryness and whether a P patrol reads as a treat** | the four scenes fit the six-line panel | confirm sfx; no battle music on a P patrol | 129_patrol_scene.png | not run |
| 215 | Debug patrol selector (M110, dev build) | Debug → **Next patrol** Left/Right through Random / Normal / Golden Goose / Lore / Chests / P; pick P, close the menus, walk a patrol down (or Trigger patrol now); then pick Normal and repeat | not run | not run | the row cycles the six labels and reads cleanly; the forced kind answers exactly once and the row is back on Random afterwards; the patrol AFTER a forced one is whatever the seed would have produced anyway (compare against a reload that never forced) — the override never shifts the sequence; Golden Goose / Lore / Chests answer as an ordinary patrol until M111/M112 | cursor + slab on the stepper | move/confirm sfx | 128_debug_patrol_rows.png | not run |
| 216 | Debug event selector (M110, dev build) | Debug → **Next event** Left/Right; pick e.g. healing_spring; face the next plain event marker in the dungeon (a shrine, a peddler, an altar - anything but a relic or an elite challenge); resolve it; face another | not run | not run | the row cycles Random plus every substitutable kind by its flavor id (no shrine / merchant / elder root / surveyor / miner's cache / duck peddler / relic / challenge in the list); the moment a plain event is FACED its marker glyph, footer prompt and panel all become the chosen kind and resolve as it; the row is back on Random; the next event is what the dungeon generated; a relic room or a challenge is never hijacked | glyph + prompt agree | interact sfx | n/a | not run |
| 217 | The Golden Goose (M111) | dev build: Debug → **Next patrol = Golden Goose**, close the menus, Trigger patrol now; fight it three ways - let it run, kill it inside three rounds (a level-up'd party with a burst skill or two), and start with a status-immune or cleansing kit; also meet one naturally in a long run | not run | not run | a single golden goose stands alone in the centre - no crystal shatter, no boss music, the ordinary patrol fight; it acts FIRST: one wing-beat line and every standing hero is Blinded + Silenced + Poisoned at once (a downed hero untouched; the debuff tint flashes once), then it tucks its head (guards), then it takes wing - the "fled" line, no jingle, no XP, no gold, no escape counted, the Patrol chip back at 100; killing it inside those turns pays **2000 gold** and the same XP a normal patrol here pays, no drop; hits mostly miss it (Evasion) and spells bounce (Reflect) or are warded; a cleanse the moment the dust lands makes the hunt fair. **Judge whether the hunt is exciting and whether three turns is the right window** | the lone-foe centre slot; the three status glyphs in the column | debuff sfx on the dust; no fanfare on the flight | 130_battle_golden_goose.png | not run |
| 218 | The Golden Goose's numbers (M111) | after row 217: open the debug Lifetime rows and the scoreboard; reload the entry autosave and trigger the same patrol index again | not run | not run | geese met / defeated / escaped tally in the ledger (M116 shows them); the gold shows in the purse at once and in the run's gold; a reload produces the SAME goose at the same patrol index with the same XP payout; new score entries still tag generation v24 (the rules bump is v19, not a generation change) | n/a | n/a | n/a | not run |
| 219 | The Jester's lore question (M112) | dev build: Debug → **Next patrol = Lore**, Trigger patrol now; answer right once, wrong once, strike the Jester once, sweep the field once (a Dragon or an all-foes skill), and Escape once; try Guard, Item and a heal skill while deciding | not run | not run | the Jester (the class's own face) stands centre with a question above the field and two answer boxes beside it; Guard and Item are greyed with a reason, ally-facing skills grey with N/A and "Not here"; striking the right answer pays **500 gold** with the victory fanfare; the wrong one shows the Jester's mock line with the **mocking jingle** (a short sour phrase, not the defeat dirge); striking the Jester or sweeping the field knocks the striker out **for real** (still KO'd back in the dungeon; a lone survivor doing it is a wipe); every pick costs one battle turn; nothing enters the bestiary; Escape steps away as any patrol. **Judge the Jester's dryness, the jingle, and whether the punishment reads as fair** | the prompt strip fits two lines; answer boxes fit the longest answer | mock jingle / victory fanfare | 131_lore_question.png, 132_lore_result_mock.png | not run |
| 220 | No spoilers in the questions (M112) | a fresh save at town 1: force a dozen Lore patrols (reload and re-trigger); then at the highest town you have | not run | not run | at town 1 only the four tier-1 questions ever come up (every question before a repeat, then the cycle again); higher towns add their tiers; the King's-name question never appears before the King has fallen; the same patrol index after a reload asks the same question with the answers on the same sides | n/a | n/a | n/a | not run |
| 221 | The treasure chests (M112) | dev build: **Next patrol = Chests**, Trigger patrol now; open each of the three across reloads/re-triggers; sweep the field once | not run | not run | three closed chests, the warning "Three chests. One of them is lying." above; the paying chest gives **500 gold** or a piece of worn gear sold at this town (never legendary, never a relic/heirloom/consumable/scroll) with the fanfare; the empty one mocks; the lying one **becomes the Mimic** the instant it is struck - boss music, the Mimic alone at the centre, "The lid was a lie.", your very blow landing as round one's opener, then a normal boss fight; a sweep wakes the Mimic too; a reload re-shuffles nothing | the Mimic's 36x36 canvas clears the rows | boss music on the reveal | 133_chests.png, 134_mimic_revealed.png | not run |
| 222 | The Mimic's weight and payout (M112) | beat the Mimic at two different towns; lose to it once; escape it once | not run | not run | it fights like a dungeon boss of that floor (power smash, sunder, a venom bite, counter-attacks; not trivial, not a superboss - **judge**); victory shows the spoils panel with **boss-level XP + 500 gold**, no drop, and the Mimic appears in the bestiary; a loss is the ordinary wipe; an escape counts as any escape; the ledger's chest/mimic counts move (M116 shows them) | spoils panel | victory jingle | n/a | not run |
| 223 | Cutscene stages in town (M113) | start a New Game (the prologue); arrive at a new town; after the King, the finale and the roadside Stranger; Debug → Play cutscene for any scene | not run | not run | every town scene plays over the mountainous panorama (night sky, mountain bands, a keep and roofs at the far edges) instead of the flat sky/floor; the party lineup, the goose, and in the finale the King and Dragon stay clearly readable over it (a dim strip sits behind the actors); the dialogue panel is as legible as before. **Judge restraint - the art must not compete with the actors or the text** | text over the framed panel only | n/a | 112_cutscene_dialogue.png, 114_cutscene_finale.png | not run |
| 224 | Cutscene stages in the dungeons (M113) | in a Keep, a Mine, a Forest and (town 7) a Goosy run: meet a Stranger story room (A Familiar Hood) and a P patrol scene (Debug → Next patrol = P) | not run | not run | the scene plays over THAT theme's stage - parapets and arches / crystals and supports / trunks and roots / reeds and still water - regardless of which tale it is; the same tale on a different theme shows a different stage; actors and text stay dominant | n/a | n/a | 135_cutscene_stage_keep.png, 136_cutscene_stage_mine.png, 137_cutscene_stage_forest.png, 138_cutscene_stage_goosy.png | not run |
| 225 | Font readability (M114) | play at 1x window scale and under the CRT filter: town labels, the battle HUD (HP/MP numerals, status tags, footer hints), the party sheet, the scoreboard, a dungeon footer prompt, a shop list, the Help screen, a cutscene; then the pseudo-localized captures' screens if you can reach them (accented names in party creation) | not run | not run | the same typeface, redrawn taller and clearer: `I l 1`, `O 0`, `C G`, `c e`, `r n m`, `u v` tell apart at a glance; paired quotes read as quotes; every caption is at least 9 px (nothing 8 px survives); lines never overlap their neighbours (descenders now exist); nothing is clipped or ellipsized where it was not before; column layouts (party sheet, scoreboard) still align. **Judge readability at 1x and under CRT; report any screen that got worse** | every fitted site clean under the capture lint | n/a | 139_font_specimen.png, 65_debug_menu.png, 23_battle_targeting.png | not run |
| 226 | Reflow, not shrink (M114) | the screens the capture lint flagged and the note lists as reflowed: check each at 1x | not run | not run | the reflowed surfaces read naturally (a wider budget, a compact fallback or a wrap - no smaller text): nothing overlaps its backing frame, hints and chips keep their backing strips over art | n/a | n/a | n/a | not run |
| 227 | The Last Dragon's face (M115) | with all twelve curios: fight the Dragon (dev: God Mode helps); bring it under 5% so it clones itself; target the clone and the Dragon in turn | not run | not run | the Dragon reads as a DRAGON at a glance - horned one-eyed head, snout, neck, one raised wing with daylight between its fingers, claws, a banded torso, a curling tail; the clone wears the SAME art (no beast placeholder), sits centre-top above the Dragon, and neither sprite, meter, status line nor focus bracket ever overlaps the other; the clone still counts as no boss (no bestiary entry of its own, the Dragon's defeat counted once). **Judge the silhouette against the other bosses** | the two 36x36 sprites clear each other | boss music | 140_dragon_clone.png, 141_dragon_target.png | not run |
| 228 | Formation with tall units (M115) | any boss fight with a full court (the Rush Tyrant, a Guild Master's court, an Endless boss wave), and a plain 5-enemy patrol | not run | not run | rows sit exactly where they did before (the boss at centre with the two rows above lifted by the old headroom); nothing climbs off the top of the screen; a 5-enemy fight still starts higher; target cycling walks top-to-bottom | no overlaps anywhere | n/a | 18_battle_boss.png, 31_battle_high_town.png | not run |
| 229 | The End-game Summary unlocks once (M116) | on a save that has not finished the story: beat the King, walk to town 7's eastern roadside, play the finale and make the keepsake choice; return to town; then quit and reload; then walk back to P | not run | not run | the summary appears by itself exactly once, right after the finale pops back to town (a finale achievement toast, if any, dismisses first) - the victory stinger plays and the Result loop follows; a reload never re-shows it; P at the roadside now offers **Talk** / **End-game Summary** (Cancel steps away); Talk still tells the next joke in the cycle; the summary from P opens straight on the Result loop; leaving it restores the town music | the modal's two rows | victory stinger then Result | 148_stranger_choice.png | not run |
| 230 | The six pages (M116) | in the summary: cycle the pages both ways with the board-cycling keys; scroll each page; read the Bestiary page against your Bestiary | not run | not run | Overview / Heroes / Combat / World / Bestiary / Patrols, each named with n/6, wrapping both ways; every page scrolls (nothing crammed); values right-aligned with thousands grouping, play time as Hh MMm; Heroes shows each member with their current name, class and level and their eleven ledger rows plus skills known; World shows seven town blocks then Economy and Exploration; the Bestiary lists exactly your bestiary - met foes with "Defeated: N", unmet ones as ? ? ?, and opening the summary never marks anything as met; Patrols shows the geese / lore / chest breakdown. **Judge whether the summary is fun to read and whether anything is missing** | every row fits its column (the capture set covers 7-digit counts and 12-char names) | move sfx on cycle | 142_summary_overview.png ... 147_summary_patrols.png | not run |
| 231 | Live counters and old saves (M116) | after the summary has unlocked: fight a battle, buy something, meet a patrol, then reopen the summary from P; then load a save made before the M109 ledger existed and finish its story | not run | not run | the numbers moved (battles won, gold spent, patrols met, the Bestiary counts); the pre-ledger save's Overview opens with "Lifetime tracking began with this version" and its counts start from zero except the biggest hit carried over | n/a | n/a | n/a | not run |
| 232 | CRT defaults on a fresh file (M117) | delete (or rename) `settings.json` in the user data dir and launch; or Settings → Reset settings and bindings | not run | not run | Settings → Display reads **CRT Strength 2**, **CRT Curvature 0**: faint scanlines and mask on a perfectly flat rectangle; a file written by an earlier build keeps its own values until reset. **Judge whether 2 over flat glass is the right fresh-install look** | n/a | move sfx on adjust | 60_settings_display.png | not run |
| 233 | The Controls page in Release (M117) | build the Release preset; Main Menu → Controls; then the same page in the Debug build | not run | not run | Release: eight rows, no "Toggle debug overlay / F1" line, the frame closing under the last row; Debug: the F1 row still shows (the key works there) | the frame fits its rows | n/a | 02_help.png (Debug) | not run |
| 234 | Blackjack stakes (M117) | a card table with 1000+ gold, then with about 120 gold | not run | not run | the bet pick offers Bet 10/25/50/100/250/500/1000g in one unscrolled window; with 120 g only 10/25/50/100 show; a won 1000 g hand pays 2000 back, a push returns 1000, a loss keeps it; the table minimum is still 10 g | seven rows fit the modal | card sfx | 149_blackjack_stakes.png | not run |
| 235 | The Jester waits (M117) | a party with one Jester and three others; dev build: Debug menu → Next patrol: Chests (then Lore) → Trigger patrol now | not run | not run | the Jester never acts while the chests or the question stand — the first controllable member's turn opens the choice; no damage number ever appears on a chest or an answer; a reward or a mock ends the fight with the Jester having done nothing; when the Mimic wakes, the Jester joins the boss fight normally (its quips resume); the Lore Jester struck by a controlled member still KOs the striker | the prompt strip stays up | mock / victory / boss music as before | n/a | not run |
| 236 | An all-Jester party decides (M117) | four Jesters (create them at New Game); Chests and Lore patrols through the debug one-shots, several times | not run | not run | the Jester's own turn resolves the encounter: a swing or hostile skill at a chest opens it (or wakes the Mimic, with that action as the opening blow), at an answer gives it, at the Lore Jester earns the KO; an all-foes skill is the sweep (Lore: the KO; Chests: the Mimic); a Mend turn heals an ally and the encounter keeps waiting for the next Jester; no quip rides over a resolved decision; a last-standing Jester's punishment is a real Defeat, not a stall | n/a | as row 235 | n/a | not run |
| 237 | Every event has its own icon (M118) | walk several floors in every theme (dev: Debug menu → Next event: each kind in turn, then face a plain event); include a rite room per theme, a Royal Relic, the two dens, the Surveyor and Dragonform rooms | not run | not run | every event marker is a distinct 12×12 icon that reads at 1× — never a bare letter or an empty spot (the nine formerly invisible kinds: goose polymorph, sacrifice, level altar, Stranger story, token exchange, patrol reset, reels, blackjack, goosy flock); the flavor panel shows the same icon left of its title. **Judge whether each icon says what the event is before the prompt does** | icons inside their tile; the panel title clear of the icon | n/a | 150_event_icons.png, 86_event_flavor.png | not run |
| 238 | The glint (M118) | stand in a room with an unresolved event; resolve it; return | not run | not run | a small crystal pip flickers above the icon while the event is unresolved; it never covers the icon, never strobes (two frames on the shared clock), and vanishes with the marker once the event resolves. **Judge restraint** | n/a | n/a | n/a | not run |
| 239 | The service interiors (M119) | visit the Inn, Item Shop, Equip Shop, Training Hall, Scoreboard and Guild in town 1, then in a high town (5–7) | not run | not run | each room reads as a painted place at a glance (the lamplit window and hearth, the jars and herb bundle, the forge and anvil, the dojo screen and target, the pillars and banners, the map and notices) while the frames, lists and captions stay perfectly legible — the caption row has its translucent strip; nothing loud fights the panels; the town shades still darken the higher towns. **Judge restraint: the art must set the place, never compete with the text** | every caption and list unchanged in fit | town music | 07_inn.png … 13_guild.png, 28_training_passives.png, 30_equip_shop_max.png, 89_scoreboard_4f.png, 122_training_spar_row.png | not run |
| 240 | The title scene (M119) | launch; watch the phrase pulse; open and close the menu items | not run | not run | the lake, reeds and ridge sit behind the emblem, plaque and menu without touching them; the phrase reads over its strip at every pulse step; the version stamp and footer are unchanged | the phrase strip spans the phrase | title music | 01_title.png | not run |
| 241 | The painted battle stages (M119, corrected 2026-09-16) | fights in every theme (a plain patrol and a boss), a castle challenge, then the same with High Contrast on | not run | not run | each fight stands on a painted stage that reaches the command panel: a far strip behind the top row (Keep: masonry, a wall stump, the tower's edge; Mine: the crystal drip and lintel, supports, side clusters; Forest: the canopy's underside, trunks; Castle: the plinth line, pilasters, windows; Goosy: mist over the far pond, reeds) and one continuous ground under the whole formation (flagstone courses; a rock floor with rails and sleepers; earth with a widening path and roots; paving with a dais runway; a muddy bank with puddles and a near water's edge), the M56 silhouettes at the skyline, in the margins and at the near edge; floats, status lines and meters read as before over the quiet field; under High Contrast the painting is gone but the plain floor (a dark far strip, a horizon line, a lighter ground with seams) stays under every unit. **Judge that the fight is never harder to read** | floats/status text over the field | battle/boss music | 151_stage_keep_patrol.png, 152_stage_mine_patrol.png, 153_stage_forest_patrol.png, 154_stage_plain_fallback.png, 155_stage_castle_court.png, 70_backdrop_mine.png, 71_backdrop_forest.png, 72_backdrop_castle.png, 120_battle_goosy.png, 121_battle_goosy_boss.png, 130_battle_golden_goose.png, 140_dragon_clone.png, 73_backdrop_mine_hc.png | not run |
| 242 | Battle-stage grounding (M119 correction, 2026-09-16) | in each theme's fight, then the castle and a Goosy fight, look at every row in turn: the top party row, the second, the third, the bottom row, each enemy's feet, a boss's footprint, the HP/MP meters, the floor transition behind the top row, the band's lower edge above the command panel; repeat once with High Contrast on | not run | not run | every party member visibly stands on the same ground plane (no one below the painted floor, no one hanging on a wall); the enemies and a boss stand on it too; the ground is quiet but not empty — a few seams, roots, rails or ripples explain the plane without competing with a sprite; the far strip, the ground and the near edge can be told apart; no unit looks parked on its HP bar or on a pedestal of its own; under High Contrast the plain floor still grounds everyone; in grayscale (squint) the actors still win. **Judge whether each combatant meets the environment without a bright circle or platform under it** | none | battle music | 151_stage_keep_patrol.png, 152_stage_mine_patrol.png, 153_stage_forest_patrol.png, 154_stage_plain_fallback.png, 155_stage_castle_court.png, 121_battle_goosy_boss.png, 140_dragon_clone.png, 31_battle_high_town.png, 73_backdrop_mine_hc.png | not run |
