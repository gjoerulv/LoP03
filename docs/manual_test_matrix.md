...
| 165 | Victory celebration (M71) | clear a dungeon with NO stakes penalty; clear one WITH a penalty; clear one where a member ended KO'd; beat the King, the Duck gauntlet, and the Boss Rush; run the Endless Rush; lose a challenge; celebrate several times | not run | not run | the clean clear shows the celebration FIRST (Victory! plaque, the score alone in gold, a **random dry punchline** beneath — varying across celebrations — and confetti), then Confirm → the usual detailed reckoning; the penalized clear skips straight to the reckoning; every standing member jumps at a visibly different rhythm/height, the **MVP stands named on the centre pedestal** with the gold chip (jumping higher), and the KO'd member lies horizontal, dimmed, not jumping; the King/Duck/Boss-Rush wins show the same screen with "Cleared in N turns!" before their result overlay; the Endless Rush and all losses never celebrate. **Judge the jump feel, pedestal, confetti density, and the punchline pool's voice** | headline + punchline + MVP name fit | fanfare (Result) music during the celebration | 84_celebration.png | not run |
