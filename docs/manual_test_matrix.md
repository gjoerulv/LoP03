...
> localized long-prose stress; **124** as of the 2026-08-17 fix rounds).
...
| 197 | Pond spirit & the forge (M103; modal window fix 2026-08-17) | sweep seeds until the Pond Spirit and the Hungry Forge appear; accept both; fight on; end runs each way (boss win, retreat, wipe) with a goosed member | not run | not run | the pond names a RANDOM non-goose member who fights as a bare Goose (heirloom kept, arms gone) for the REST of the run; +100 itemized as "Goose pact"; XP/levels earned while goosed survive the restore; a KO'd goose comes back KO'd; an all-geese party gets a dry refusal and keeps the event; the forge lists only BAG equipment, the piece is gone, and the NEXT victory panel shows doubled XP exactly once. the forge's pick list shows at most nine rows in a scrolling window (arrows mark the rest) under a two-line prompt - a deep bag can no longer push the box off screen. **Judge the pact's fun and the 6-8% rates** | choice modal + outcomes fit | interact/status sfx | n/a | not run |
