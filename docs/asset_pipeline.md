...
67 sprites ship: 27 normal + 25 elite + 15 boss (the last three being the
generic tier fallbacks `enemy.normal/elite.battle` and `boss.generic.battle`,
retained). `BattleState` prefers the per-id sprite and falls back to the
...
- **The boss canvas is bounded by battle layout, not taste.**
  ...
  fields 5+ enemies, which three authored boss teams do. 36 rows is therefore
