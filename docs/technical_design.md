...
Draw-order rule (town + dungeon): tiles (since M128 the town's trees and
ground and every theme's walls pick one of their authored variants per cell
through `render::tileVariant`, a pure position hash — never an RNG stream —
beside the older sparse accent pick) → world sprites (markers, then the player) → overlay pass (danger
...
list lives in `src/capture/CaptureRunner.cpp`; **195 scenes as of M131
(2026-09-22)**,
...
one-shot commits, and settings honoring. Since **M130** `start` takes an
optional windup length (the action's tier chooses 0.24 / 0.30 / 0.42 s on
Normal; Fast and Instant pass the base) — see §70.
...
- **Presentation resolution (all with base fallback).** Per-town exterior tiles
  ...
  rebinds the `MusicTrack::Town` stream). Interiors, music and the
  `grass`/`path`/`building` tiles are generated by `tools/asset_gen/` as
  deterministic palette/tempo shifts of the town-1 base (existing outputs
  byte-identical); since **M128** the trees and the ground are hand-placed per
  town instead (`tiles.town.<n>.tree.<1..4>`, `.ground.<1..4>`, town 1 included
  — the tinted tree/ground copies are gone; see §69); the presentation lint
  requires every town to resolve its tiles, interiors, and music.
...
- **Triggers**: new game (PartyCreationState::begin, pushed over the
  ...
  Town-8 roadside (`kGooseNpcTileX/Y`; gated since M131 on the save's
  own `castleRecords.kingDefeated` through `game::strangerAtRoadside`,
  §71 — the profile flag until then), who stays for replays.
...
## 69. M128 — tile redesign: the ladder's trees, seasonal ground, wall variants

Presentation-only; no version motion, no layout, collision, generation or
RNG-stream change.

- **Variant placement**: `render/TileVariant.hpp` (pure, header-only) —
  ...
  `dungeon::themeEventHash` uses (duplicated so `render/` never depends on
  `dungeon/`); `tileVariant(seed, x, y, salt, weights, count)` walks
  cumulative buckets. A linear `(x*a + y*b) % n` pick stripes diagonally;
  `tests/test_tile_variant.cpp` sweeps rows, columns and both diagonals for
  any period ≤ 8 and pins the weights within 3 % over 10 000 cells.
- **Town**: `TownState::render` resolves the town's `tiles.town.<n>.tree.<1..4>`
  and `.ground.<1..4>` ids ONCE per frame (callers cache nothing across a
  reload) and picks per cell with seed 0 and the town index as the salt —
  the identical ring geometry reads as a different organic pattern in every
  town. Town 1 takes the per-town path too. Fallback: the variant → variant
  1 → the M32 per-town id → the base id → the coloured rectangle.
  `buildTown` never places `Tile::Grass`, so the grass tiles and the flowers
  accent are dead paths (left as they are).
- **Dungeon**: `DungeonState::render` picks `tiles.<theme>.wall.<1..4>` for
  `Tile::Building` cells with `seed = dungeon_.seed`, `salt =
  kSaltWallVariant + currentRoom_` (weights 55/20/15/10), the four ids
  resolved once per frame so the cell loop allocates nothing; fallback
  variant 1, then the legacy `wall`.
- **Assets**: 72 hand-placed 16×16 grids in one RNG-free generator section
  (`Save-TileGrid`: exactly 16×16, fully opaque, no outline pass); three
  foliage ramps (art bible §2); the twelve M32 tinted tree/ground copies
  deleted (`ShadeCopy` is RNG-free, so byte-safe); the legacy
  `town_tree.png` / `town_ground.png` / `<theme>_wall.png` keep being
  generated (their lines consume the shared RNG stream) and stay the
  fallbacks. Review harness `tools/asset_gen/preview_tiles.ps1` →
  `docs/sprite_review/tiles_towns.png`, `tiles_walls.png`.
- **Capture**: `183_town_2` … `188_town_7` (each sets its town, clears the
  black market and the guild records so no pending perk offer modals over
  the exterior). **Tests**: `tests/test_tile_variant.cpp` (`[m128]`, incl.
  the 16×16 PNG header check), the presentation lint's town and theme loops.

## 70. M130 — short attack & skill animations

Presentation-only; no version or schema motion; the Simulator never sees it.

- **Model** (`render/ActionTier.hpp`, pure): the family is the M121
  `content::SkillKind` (a basic attack takes `damageKindFor(weaponElement)`;
  an item is Heal when it restores, else Buff/Debuff by its side; a summon
  has none); `actionTierForSkill` derives Minor / Major / Grand from
  target, category and power (damage only — support never escalates);
  `actionFxEnabled(speed)` is the ONE place the owner's Fast rule lives
  (Normal only); `actionWindupSeconds` returns the tier's windup on Normal
  and `kWindupBase` otherwise; `actionFrames` (3/4/6), `actionTailSeconds`
  (0.20/0.30/0.45), `kActionFrameTime` 0.07, `frameIndex` (holds the last
  frame). `tests/test_action_tier.cpp` pins the shipped skills' tiers, the
  timing table and that every burst fits inside the Normal settle.
- **Sequencer**: `start(..., float windupSeconds = kWindupBase)`; the impact
  beat is never varied. No new stage and no elapsed-time getter —
  `BattleState` owns `actionFxT_` (reset in `afterAction`, advanced beside
  `seq_.update` inside the capture-freeze guard, so captures stay
  byte-exact) and `actionWindup_` (0 when there is no impact beat).
- **Effects** (`render/ActionFx.{hpp,cpp}`, raylib): `drawActionCast`
  (motes over a caster during the windup; the caster's sprite rises 2 px)
  and `drawActionBurst` (one motif per family in 2×2 blocks, frames then a
  linear-fade tail; a negative or expired clock draws nothing);
  `actionFamilyColor` (the M91 hues, success, gold, magic). `drawUnit`
  draws the burst on every unit in `fxTargets_` (staged in `stageNumbers`:
  any HP delta; a status-only action marks the touched side or its one
  target, `actionTarget_`) with `burstT = actionFxT_ - actionWindup_ -
  ordinal * 0.06` (the Grand stagger); the lunge is
  `actionLungePixels(tier)` with a held peak for Grand; the M91 accent is
  drawn only when the ActionFx is off. The Grand wash reuses `AoeTint`
  (`aoeTintAlpha` gained a `cap` argument: 0.20 for Grand) in the family's
  hue.
- **Wiring**: `setActionAnim(skill, weaponElement, hitsAll, target)` /
  `setActionAnimItem` beside every `fxElement_` assignment — the five
  hit-dealing paths, the decision resolution, the status ticks, the element
  capture — each path resetting to neutral first.
- **Capture**: `captureActionFxAt(skillId, atSeconds)` (a single-strike
  attacker for the empty id; the party wounded first for an ally skill);
  scenes `189_anim_minor_strike`, `190_anim_grand_inferno`,
  `191_anim_grand_chain_lightning`, `192_anim_heal`.

## 71. M131 — owner batch 4: the roadside gate, the Iron Man Dragon bar, the Alarm

Three owner items from the M128–M130 review session (2026-09-22). No
version or schema motion: content stays v1 (`alarm` is a new value of the
existing `effect` enum — additive, the M120 `mpAmount` precedent),
generation stays 25 (the Alarm never enters a pool), rules 19, save v1.

- **The roadside gate** (`game/Cutscenes.hpp` `strangerAtRoadside`, pure):
  `TownState::gooseNpcHere` reads the save's own
  `castleRecords.kingDefeated` (town 7 only), never the profile's
  class-unlock flag — the M97 gate opened the finale's door for any new
  party on a profile that had ever beaten the King. The profile flag keeps
  its two other jobs untouched: the reward classes (M45) and the post-King
  lore questions (M112, `makeLoreEncounter`). `TownState` no longer
  includes the profile store.
- **The Dragon bar** (`game/IronMan.hpp`): `kBarredClassId` ("dragon"),
  `classBarred(id)`, `hasBarredMember(party)`, the capsule suffix and the
  roster note; `kRules` grew to five (the bar is the fourth paragraph).
  `PartyCreationState::classBarred` greys the class like a locked one;
  `anyBarredSelected` refuses Begin with the M45 error tone and the note
  under the roster says why; the details sheet ends with the same line.
  `achievementMet` folds `!hasBarredMember` into the three `iron_*`
  predicates. Older saves are unaffected by construction (an Iron Man
  party is never saved, so none can be loaded).
- **The Alarm** (`ConsumableEffect::Alarm`, JSON `alarm`;
  `game/ItemUse.hpp` `itemIsAlarm` + the town and member refusal lines):
  the loader accepts it on a consumable only and rejects a battle target or
  a status rider on it; `itemShopBuyIds` stocks it by the ordinary town
  window (`minTown: 4`); `itemShopCategoryRank` ranks it an oddity;
  `buildPools` skips it (never a chest, never a peddler offer);
  `BattleState::consumableIds` never lists it, and `Battle::useItem` treats
  it as `None` should content ever hand one to a battle. The use path:
  `InventoryState(…, inDungeon)` (true only from `DungeonMenuState`) spends
  one, sets the runtime one-shot `AppContext::alarmRung` and pops both
  menus; `DungeonState::update` consumes it on its next tick and calls
  `triggerPatrol()` — the M110 dispatcher, the seeded kind for this patrol
  index, the M109 tallies — so a rung patrol is indistinguishable from a
  walked one; the dungeon constructor clears a stale flag. CrystalForge
  picks the value up through `consumableEffectIds()`.
- **Captures**: `193_class_select_iron_man_dragon`,
  `194_items_alarm_dungeon`, `195_items_alarm_town` (**195 scenes**);
  `169_new_game_iron_man` re-read for the fifth paragraph.
- **Tests**: `tests/test_owner_batch_4.cpp` (`[m131]`);
  `tests/test_editor_enum_lists.cpp` (six effect ids).
