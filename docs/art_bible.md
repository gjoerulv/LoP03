...
- **Enemies:** hunched/bestial shapes vs. the party's upright ones; elites
  ...
  crystal glow. Danger is shape+size+accent, never hue alone. **(M26/M29:
  realised as 37 distinct per-id battle sprites — 22 normal + 9 elite at 24×24,
  6 bosses at 32×32 — generated by `generate_textures.ps1`; a presentation-lint
  guard fails the suite on any enemy/boss id lacking its own sprite.)**
