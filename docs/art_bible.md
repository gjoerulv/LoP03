...
> **Reconciliation (2026-08-17; closes the 2026-08-07 audit debt).** The
> generator's 68 distinct hex literals were censused against this section:
> 43 are the recorded ramps above, and every one of the 25 survivors is
> attributable — no orphan hues, nothing retinted. The survivors, now
> sanctioned as three named groups plus one pixel:
>
> 1. **Service-interior gradient bases (12, M27 §6):** the six `New-Bg`
>    top/bottom pairs (Inn `#241A18/#150F0D`, Item Shop `#16182A/#0D0F18`,
>    Equip Shop `#1E2028/#101218`, Training Hall `#241618/#140D0F`,
>    Scoreboard `#1A1626/#100C18`, Guild `#16201A/#0D140E`) — the "distinct
>    dark tinted gradient" §6 mandates, all darker than `night1` and used
>    only as full-screen grounds.
> 2. **M32 town-ladder tint constants (6):** `#26364E #2C3A2A #1E2A44
>    #2E2442 #3A2030 #280E1A` — parameters of the approved ColorMatrix
>    shading transform (towns 2–7), never painted directly.
> 3. **±1-step derivations (7):** `#322E3E` (keep-wall mortar speckle,
>    night3↔stone1), `#3A2E20` (bark shadow, earth1 −1), `#57503C` (earth
>    speckle, desaturated earth midpoint), `#E8C56A` (warm window light,
>    gold −1 warm), `#7B50BC` (violet −1: the elite/boss overworld disc),
>    `#B24040` (danger −1: the marker disc), each inside the allowance the
>    rules above already grant.
> 4. `#FFFFFF` — the save crystal's single specular pixel above `white2`.
>
...
- **Gear icons (M81): 10×10**, one per `iconCategory` (11 shipped), drawn as
  ...
  silhouettes.
- **Type (M25; M114):** the original bitmap font lives on a **6×9 master
  cell** — cap height 7, x-height 5, two descender rows, accents in the top
  two rows (accented capitals compress to 5-row bodies) — drawn as
  hand-placed grids in `generate_font.ps1`, never traced. The caption floor
  is **9 px**; body is 10; headings scale from the 2× atlas at 20. Layout
  answers a tight fit by reflow, never by text below the floor.
...
- **Enemies:** hunched/bestial shapes vs. the party's upright ones; elites
  ...
  **100 sprites — 73 `enemy.*` at 24×24, 27 `boss.*` at 36×36**
  (M84 added the 7 Guild Masters and their 12 courts, M85 the Last Dragon —
  redrawn in M115 as a dragon silhouette: horned one-eyed head, neck, one
  raised wing with daylight between its fingers, foreclaws, a stone-banded
  torso, a curling tail; 2026-08-17 the Goosy Gauntlet's 8 pond fowl + 3
  bosses; M111 the Golden Goose, M112 the Mimic — all under the same
  §5/§5b rules) — generated by
...
- **Cutscene stages (M113):** five full-screen scenes behind the Hooded
  ...
  reeds/still water). Rules: darker than every actor ramp, no signal
  colours, the **actor band (y 36..106) kept quiet** — motifs live above
  it (sky, peaks, canopy, parapets) and below it (the floor), with only
  far, low-alpha silhouettes at the band's edges; the state adds a dim
  strip over the band and the framed dialogue panel backs the text. A
  missing stage falls back to the M97 flat sky/floor fills.
...
- The goose emblem (32×32, a goose riding still water with one cyan glint)
  marks the title screen and is the source of the executable icon
  (`generate_icon.ps1` → `packaging/arepgeese.ico`) since the 2026-08-17
  rebrand follow-up; the crystal emblem stays shipped as its fallback.
...
- **Goosy Gauntlet** (2026-08-17) — still dark water underfoot with faint
  ripple bands, woven reed-palisade walls bound by two earth bands,
  parted-reed doorways over a water sill, and a nest-shrine accent (woven
  nest, pale eggs, one cyan-marked egg). Identity: a flooded pen — thin
...
- **Goosy Gauntlet** (2026-08-17) — a soft watery reed rustle over a barely
  beating still-water swell, named by **distant double goose honks**
  (300–340 Hz, brassy second harmonic, quick attack — far below the
  birdcall band the rule below reserves), water plops, and one wing flap.
  Identity: an occupied wetland. Its music is the `dungeon_goosy` waddling
  strut (G dorian, 116 BPM, A/B/breakdown/A per §9).
